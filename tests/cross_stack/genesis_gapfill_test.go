...
func TestExplicitParamsSmokeTest(t *testing.T) {
	...
	t.Run("VestingRewards", func(t *testing.T) {
		...
		require.Equal(t, "0", p.BlockReward, "transaction-presence block minting is retired")
		require.Equal(t, "0", p.FloorReward, "automatic reward floor is retired")
		require.Zero(t, p.EmptyBlockRewardRate, "empty-block minting is retired")
		require.Zero(t, p.FounderShareBps, "identity-based founder tap is retired")
		require.Empty(t, p.FounderAddress, "retired founder address must stay empty")
