	...
	specs := []paramSpec{
		{
			...
			validate: func(v interface{}) error { return v.(*knowtypes.Params).Validate() },
		},
		{
			name:     "alignment",
			defaults: &alignmentDefaults,
			validate: func(v interface{}) error { return v.(*aligntypes.Params).Validate() },
		},
		{
			name:     "capture_defense",
			defaults: captypes.DefaultParams(),
			validate: func(v interface{}) error { return v.(*captypes.Params).Validate() },
		},
	}
