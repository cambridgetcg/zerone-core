...
func requireNoSchedulerBankBalances(
	...
	for _, namespace := range []struct {
		...
		for _, balance := range bankGenesis.Balances {
			// The SDK accepts equivalent uppercase Bech32 encodings. Compare
			// the decoded address's canonical form, not the source JSON spelling.
			balanceAddress, err := sdk.AccAddressFromBech32(balance.Address)
			if err != nil {
				return fmt.Errorf("decode bank balance address for scheduler preflight: %w", err)
			}
			if balanceAddress.String() != address || balance.Coins.IsZero() {
				continue
			}
			if namespace.fresh {
				return fmt.Errorf(
