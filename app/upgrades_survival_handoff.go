...
func requireSurvivalHandoffTransitionOwner(name string, fromVM, targetVM module.VersionMap) error {
	read := func(vm module.VersionMap) ([2]uint64, error) {
		...
		if !kOK || !vOK || (pair != [2]uint64{6, 2} && pair != [2]uint64{7, 3} && pair != [2]uint64{8, 3} && pair != [2]uint64{9, 3}) {
	...
	if (from == [2]uint64{7, 3} && target == [2]uint64{8, 3}) || (from == [2]uint64{8, 3} && target == [2]uint64{9, 3}) {
...
func (app *ZeroneApp) validateSurvivalHandoffStartupVersions(ctx sdk.Context, vm module.VersionMap, latest int64) error {
	...
	if reflect.DeepEqual(compiled, reviewNeutralityTargetVersionMap()) {
		return app.validateReviewNeutralityStartupVersions(ctx, vm, latest)
	}
