...
func setupKeeperWithBankAndGenesis(t *testing.T, bk *mockBankKeeper, sk *mockStakingKeeper, gs *types.GenesisState) (keeper.Keeper, sdk.Context) {
	...
	// Initialise through the validated v2 boundary, then reproduce any legacy
	// v1 parameter bytes directly. This keeps tests of inert compatibility APIs
	// honest without weakening production SetParams or InitGenesis.
	legacyParams := proto.Clone(gs.Params).(*types.Params)
	validGenesis := proto.Clone(gs).(*types.GenesisState)
	validGenesis.Params.FounderShareBps = 0
	validGenesis.Params.FounderAddress = ""
	validGenesis.Params.BlockReward = "0"
	validGenesis.Params.FloorReward = "0"
	validGenesis.Params.EmptyBlockRewardRate = 0
	k.InitGenesis(ctx, validGenesis)
	if !proto.Equal(legacyParams, validGenesis.Params) {
		bz, err := proto.Marshal(legacyParams)
