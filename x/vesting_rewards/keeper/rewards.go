...
// RouteFees intercepts transaction fees before x/distribution sweeps them to
// validators. It routes the configured research and development slices; the
// contributor and protocol slices remain in fee_collector for ordinary Cosmos
// distribution. Must run in BeginBlock before x/distribution's BeginBlocker.
...
// injects legacy non-zero values. Historical records remain queryable. This
// claim is limited to the retired path; other disclosed issuance families have
// their own activation and evidence boundaries.
