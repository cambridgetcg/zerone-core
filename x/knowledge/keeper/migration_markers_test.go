...
func TestMigrationMarkerValidationAndIdempotence(t *testing.T) {
	...
	require.ErrorContains(t, k.WriteMigrationMarker(ctx, "founder", ""), "value cannot be empty")
	...
	require.NoError(t, k.WriteMigrationMarker(ctx, "founder", "migrated"))
	require.NoError(t, k.WriteMigrationMarker(ctx, "founder", "migrated"))
	value, err = k.ReadMigrationMarkerChecked(ctx, "founder")
	require.NoError(t, err)
	require.Equal(t, "migrated", value)
	value, found, err = k.ReadMigrationMarkerPresenceChecked(ctx, "founder")
	require.NoError(t, err)
	require.Equal(t, "migrated", value)
	require.True(t, found)

	err = k.WriteMigrationMarker(ctx, "founder", "forged")
	require.ErrorContains(t, err, "conflicts")
	require.Equal(t, "migrated", k.ReadMigrationMarker(ctx, "founder"),
...
func TestMigrationMarkerStoreErrorsPropagate(t *testing.T) {
	...
	_, err := getFailureKeeper.ReadMigrationMarkerChecked(context.Background(), "founder")
	require.ErrorIs(t, err, getErr)
	_, _, err = getFailureKeeper.ReadMigrationMarkerPresenceChecked(
		context.Background(),
		"founder",
	)
	require.ErrorIs(t, err, getErr)
	require.ErrorIs(t, getFailureKeeper.WriteMigrationMarker(
		context.Background(),
		"founder",
		"migrated",
	), getErr)
	require.Panics(t, func() {
		getFailureKeeper.ReadMigrationMarker(context.Background(), "founder")
	})
	...
	require.ErrorIs(t, setFailureKeeper.WriteMigrationMarker(
		context.Background(),
		"founder",
		"migrated",
	), setErr)
...
func TestMigrationMarkerPresenceDistinguishesHistoricalEmptyValue(t *testing.T) {
	...
	value, found, err := emptyValueKeeper.ReadMigrationMarkerPresenceChecked(
		context.Background(),
		"founder",
	)
	require.NoError(t, err)
