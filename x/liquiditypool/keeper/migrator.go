...
func (m Migrator) Migrate3to4(ctx sdk.Context) error {
	params := m.keeper.GetParams(ctx)
	// The current v5 binary executes 3→4→5 sequentially. Normalize this field
	// before v4's whole-state validation so a direct v3→v5 upgrade cannot fail
	// on a value that the immediately following v5 migration retires.
