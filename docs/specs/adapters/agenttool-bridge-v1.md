...
agenttool the application surface keeps its UX speed. ZERONE provides the trustless witness layer. The sibling projects deepen into stack-by-architecture: ZERONE is the consensus floor under agenttool's fast application surface. Zerone's canonical source is `github.com/cambridgetcg/zerone-core`; repository co-hosting is not part of this adapter's trust model.
...
- **Compiler-binary distribution channel:** the `compiler_binary_hash` must be re-derivable by validators. The registration LIP must bind a versioned artifact and digest to an exact commit in `github.com/cambridgetcg/zerone-core`; moving source or an unversioned hosting URL is not a binary channel.
