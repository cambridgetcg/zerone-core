...
Vesting-rewards v2 removes transaction-presence block minting. Its former
schedule and coupling fields are inert wire metadata; no transaction count,
height, proposer, or validator-count condition can mint through that path.
Training disbursement and contribution-challenge bonus minting are
release-sealed.
...
fields and automatic-reward amount fields to zero/empty, contains no founder
payout path, and performs no transaction-presence mint. The v1→v2 migration
