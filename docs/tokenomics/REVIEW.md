...
>
> **⚠️ Frozen historical review; not current source truth (superseded at
> atomic H1).** This review captured the 2026-02-23 design and later interim
> annotations. H1 retires transaction-presence block issuance, the founder
> auto-split, and the liquidity protocol skim. Use
...
atomic H1 retires that proposer-controlled trigger.

### 2. Founder sub-share → RETIRED AT H1

The launch setting was dormant because no founder address existed. H1 fixes
...
### 3. Transaction-presence reward → RETIRED AT H1
...
**Resolution:** H1 retires both empty and transaction-bearing automatic block
...
### 4. Historical decay curve → SUPERSEDED AT H1
...
2. **When does the cap bind?** Automatic block issuance is retired at H1.
...
6. **Can governance break the economics?** Governance still controls real-fee
   ...
   upgrades. H1 prevents ordinary Params from restoring the founder or
   liquidity taps, but governance capture remains a broader chain risk.
...
For a post-H1 testnet, priorities are fee-routing integrity, LP reserve/supply
