...
The exact H1 binary retains vesting_rewards v1 and must not advertise the later
founder handler. Native pool creation and oracle use are separate governance
actions after H1; they are not another software-upgrade height. The separate
`founder-renunciation-v1` release later owns vesting_rewards v1→v2.
...
Founder and automatic-reward retirement are source-preserved but activate
later through `founder-renunciation-v1`, not as an H1 vesting migration.

This H1 boundary makes LP income compensate contributed capital and pool risk
without a protocol skim. The later H2 boundary completes the broader rule by
retiring the founder percentage and transaction-presence mint. See
...
reward settings at launch. H1 migrates liquidity state only. Do not edit or
...
liability. The later vesting_rewards v1→v2 migration clears founder and
...
- the H1 binary reports liquiditypool v5 and vesting_rewards v1, recognises H1,
  and does not recognise a later founder handler;
...
The founder share/address, automatic-reward zero fields, and vesting_rewards
version 2 are acceptance criteria for the later founder release.
