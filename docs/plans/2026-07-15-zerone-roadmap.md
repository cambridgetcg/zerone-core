...
> below is not actionable. Current release planning uses
> `consolidation-safety-v1` H1 targeting liquiditypool v5 while retaining
> vesting_rewards v1, followed by `founder-renunciation-v1` H2 targeting
> vesting_rewards v2. There is no `liquiditypool-safety-v2` handler. See the current
