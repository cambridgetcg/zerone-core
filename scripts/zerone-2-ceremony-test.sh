...
# Unexpected files are rejected before their contents are read, including a
# renamed mnemonic. Content scanning is tested separately in an allowed file.
...
grep -Fq 'unexpected file; the release artifact set has exactly four allowlisted files' "${TMP}/secret.log" || \
  fail "renamed mnemonic filename rejection was not explicit"

cp -R "${TMP}/a" "${TMP}/secret-allowed"
printf '\n' >> "${TMP}/secret-allowed/GENESIS-MANIFEST.md"
cat "${TMP}/secret/innocent-notes.txt" >> "${TMP}/secret-allowed/GENESIS-MANIFEST.md"
if go run "${ROOT}/tools/zerone2-artifact-audit" \
  --artifact-dir "${TMP}/secret-allowed" --required-mode drill > "${TMP}/secret-allowed.log" 2>&1; then
  fail "artifact auditor accepted mnemonic content in an allowed file"
fi
grep -q 'valid BIP-39 mnemonic' "${TMP}/secret-allowed.log" || \
  fail "allowed-file mnemonic rejection was not explicit"
