...
#
# Requires: Go 1.25.14 exactly, jq >= 1.6
# ═══════════════════════════════════════════════════════════════════════════
...
check_deps() {
  ...
  command -v go >/dev/null 2>&1 || die "Go 1.25.14 required."
  [[ "$(go env GOVERSION 2>/dev/null)" == "${REQUIRED_GO_VERSION}" ]] ||
    die "Go 1.25.14 required; found $(go env GOVERSION 2>/dev/null || echo unavailable)."
}
