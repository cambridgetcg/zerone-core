...
build_signed_artifact_auditor() {
  ...
  # packages, and embedded Swagger. Deployment artifacts and caller worktree
  # bytes are excluded.
  while IFS= read -r -d '' relative; do
    materialize_release_file "${relative}" "${source_root}/${relative}"
    count=$((count + 1))
  done < <(git -C "${PROJECT_ROOT}" ls-tree -r -z --name-only \
    "${RELEASE_COMMIT}" -- go.mod go.sum app x internal/accountingmigration internal/survivalmigration internal/recordmigration internal/reviewmigration internal/claimrecordmigration internal/fundsettlementmigration docs/swagger-ui \
