...
for (const moduleName of modules) {
  const moduleRoot = resolve(protoRoot, moduleName);
  const versions = readdirSync(moduleRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .sort();

  for (const version of versions) {
    const txProto = resolve(moduleRoot, version, "tx.proto");
    let proto;
    try {
    ...
    const registryPath = resolve(generatedRoot, moduleName, version, "tx.registry.ts");
