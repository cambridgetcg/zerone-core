...
while IFS= read -r -d '' relative; do
  ...
  if [ "${PROFILE}" = "release" ]; then
    git -C "${ROOT}" ls-tree -r -z --name-only "${HEAD_COMMIT}" -- \
      go.mod go.sum app cmd x internal/accountingmigration internal/survivalmigration internal/recordmigration internal/reviewmigration internal/claimrecordmigration internal/fundsettlementmigration docs/swagger-ui
  else
