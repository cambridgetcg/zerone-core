...
- **Founder share** — **dormant on the current launch record; retired in source
  ...
  separate automatic founder sub-share. The separately scheduled
  `founder-renunciation-v1` upgrade would fix the compatibility fields at
  zero/empty and remove the path; it is not live merely because source exists.
