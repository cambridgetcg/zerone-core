...
configure_home() {
  ...
  toml_set mempool type '"flood"' "${config}"
  toml_set mempool recheck true "${config}"
  toml_set mempool recheck_timeout '"5s"' "${config}"
  toml_set mempool broadcast true "${config}"
  toml_set mempool wal_dir '""' "${config}"
  toml_set mempool size 5000 "${config}"
  toml_set mempool max_txs_bytes 67108864 "${config}"
  toml_set mempool cache_size 10000 "${config}"
  toml_set mempool keep-invalid-txs-in-cache false "${config}"
  toml_set mempool max_tx_bytes 262144 "${config}"
  toml_set mempool max_batch_bytes 0 "${config}"
  toml_set mempool experimental_max_gossip_connections_to_persistent_peers 10 "${config}"
  toml_set mempool experimental_max_gossip_connections_to_non_persistent_peers 10 "${config}"
